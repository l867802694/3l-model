...
日常更新遇到刚满足 20 日纳入窗口的新股时，如果东方财富板块或个股分类能够明确解析，系统会只追加该新股并创建新的不可变分类快照；不会覆盖已有股票的冻结分类。无法解析、上市超过 30 日仍缺失，或已有股票行业归属发生变化时，仍会阻止发布并要求人工检查。

每周六由 `.github/workflows/classification-audit.yml` 对比冻结分类与东方财富当前行业板块成分。行业归属变化等非新股增量只生成差异报告并创建 GitHub Issue，必须人工确认后才能升级分类快照。
